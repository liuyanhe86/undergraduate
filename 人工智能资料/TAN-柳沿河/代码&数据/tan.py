...
def data_split(filename, size):
    arrfFile = open(filename)
    raw_lines = [line.rstrip('\n') for line in arrfFile]
    head = []
    index = 0
    for line in raw_lines:
        if line.startswith('@attribute') or line.startswith('@data') \
                or line.startswith('@relation') or line.startswith('%') or line == "":
            head.append(line)
            index = index + 1
        else:
            break
    data_lines = raw_lines[index:]
    random.shuffle(data_lines)
    train_line = head + data_lines[:int(data_lines.__len__() * size)]
    test_line = head + data_lines[int(data_lines.__len__() * size):]
    return train_line, test_line


def processTrainData(lines):
...
def getInstancesConditionalTAN(attribute1Index, attribute1Value, attribute2Index, attribute2Value, attribute3Index,
                               attribute3Value):
    global data
    count = 0
    for line in data:
    ...
    for line in test_data:
        ...
        if p_of_y2_line > p_of_y1_line:
            ...
            if y1 == line[-1]:
                ...
                incorrectlyClassified += 1
    print("")
    print("Correctly Classified Instances: ", correctClassified)
            ...
            if attr1.Name == attr2.Name:
                ...
                for value1 in attr1.values:
                    for value2 in attr2.values:
                        no_x1_x2_y1 = getInstancesConditionalTAN(attr1.index, value1, attr2.index, value2,
                                                                 len(attributeList) - 1, y1)
                        p_no_x1_x2_y1 = getProbability(no_x1_x2_y1 + 1,
                                                       len(data) + (len(attr1.values) * len(attr2.values) * 2))
                        p_x1_x2_given_y1 = getProbability(no_x1_x2_y1 + 1,
                                                          no_of_y1 + (len(attr1.values) * len(attr2.values)))

                        no_x1_x2_y2 = getInstancesConditionalTAN(attr1.index, value1, attr2.index, value2,
                                                                 len(attributeList) - 1, y2)
                        p_no_x1_x2_y2 = getProbability(no_x1_x2_y2 + 1,
                                                       len(data) + (len(attr1.values) * len(attr2.values) * 2))
                        p_x1_x2_given_y2 = getProbability(no_x1_x2_y2 + 1,
                                                          no_of_y2 + (len(attr1.values) * len(attr2.values)))
...
def getMaximumWeightEdge(vertex_list):
    ...
    for v in vertex_list:
        for attr in attributeList:
            if attr.Name == v.Name or attr.Name == "class" or v.Name == "class" \
                    or attr.Name == "Class" or v.Name == "Class":
                continue
...
def prims():
    ...
    for attr in attributeList:
        if attr.Name.lower() == "class":
            print("")
            break
        if parent_list.__contains__(attr.Name):
        ...
        for value1 in attr.values:
            if parent_list.__contains__(attr.Name):
                ...
                for value2 in parent_of_x.values:
                    no_x1_parentx1_y1 = getInstancesConditionalTAN(attr.index, value1, parent_of_x.index, value2,
                                                                   len(attributeList) - 1, y1)
                    no_x1_parentx1_y2 = getInstancesConditionalTAN(attr.index, value1, parent_of_x.index, value2,
                                                                   len(attributeList) - 1, y2)
...
def tan():
    ...
    y1 = attributeList[-1].values[0]
    y2 = attributeList[-1].values[1]
    calculateProbabilitiesForNB()  # 计算概率
    computeWeights()  # 计算权重
    prims()  # 构造最大生成树
    calculateProbabilitiesForTAN()  # 计算概率
    correctClassified = 0
    incorrectlyClassified = 0
    for instance in test_data:
        ...
        for value in instance:
            if index == len(attributeList) - 1:
                break
            if parent_list.__contains__(attributeList[index].Name):
                parent = parent_list[attributeList[index].Name]
                parent_value = instance[parent.index]
                key1 = attributeList[index].Name + "=" + value + "|" + parent.Name + "=" + parent_value + ",Y=" + y1
                key2 = attributeList[index].Name + "=" + value + "|" + parent.Name + "=" + parent_value + ",Y=" + y2
                numerator *= probability_distributions_tan[key1]
                denominator *= probability_distributions_tan[key2]
            else:
                key1 = attributeList[index].Name + "=" + value + "|" + y1
                key2 = attributeList[index].Name + "=" + value + "|" + y2
...
def main():
    ...
    trainLines, testLines = data_split(dataSet, size=split_ratio)
    data = processTrainData(trainLines)
    test_data = processTestData(testLines)
    if learning_type == 'n':
        print("Correctly Classified Proportion: ", naiveBayes())
    elif learning_type == 't':
        print("Correctly Classified Proportion: ", tan())
